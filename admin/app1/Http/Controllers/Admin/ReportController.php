...
class ReportController extends Controller
{
    ...
    public function expense_report(Request $request)
    {
        ...
        $store_id = $request->query('store_id', 'all');
        $store = is_numeric($store_id) ? Store::findOrFail($store_id) : null;
        $deliveryman_id = $request->query('deliveryman_id', 'all');
        $deliveryman = is_numeric($deliveryman_id) ? DeliveryMan::findOrFail($deliveryman_id) : null;
        ...
        $expense = Expense::with('user','order', 'order.customer:id,f_name,l_name')
            ->when(isset($zone) || isset($store) || isset($customer), function ($query) use ($zone, $store, $customer,$deliveryman) {
                return $query->whereHas('order', function ($query) use ($zone, $store, $customer,$deliveryman) {
                    $query->when($zone, function ($query) use ($zone) {
                        return $query->where('zone_id', $zone->id);
                    });
                    $query->when($store, function ($query) use ($store) {
                    ...
                    $query->when($deliveryman, function ($query) use ($deliveryman) {
                        return $query->where('delivery_man_id', $deliveryman->id);
                    });
                });
            })
            ->when(isset($deliveryman_id) &&  $deliveryman_id != 'all', function ($query) use ($deliveryman_id) {
                return $query->where('delivery_man_id',$deliveryman_id);
            })
            ->when(isset($type) &&  $type != 'all', function ($query) use ($type) {
                return $query->where('type',$type);
            })
            ->when(isset($type) &&  $payment_method_id != 'all', function ($query) use ($payment_method_id) {
                return $query->where('method_id',$payment_method_id);
            })
        ...
        return view('admin-views.report.expense-report', compact('expense', 'zone', 'store', 'filter', 'customer','deliveryman','type'));
    }
