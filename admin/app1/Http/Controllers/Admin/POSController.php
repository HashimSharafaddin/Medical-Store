...
class POSController extends Controller
{
    public function index(Request $request)
    {
        ...
        ->when($category, function($query) use ($category) {
            // جلب المنتجات حسب الفئة أو الفئة الرئيسية
            $query->whereHas('category', function($q) use ($category) {
                return $q->whereId($category)->orWhere('parent_id', $category);
            });
        })
        ->when($keyword, function($query) use ($key) {
            // جلب المنتجات بناءً على الكلمات المفتاحية
    ...
    // إذا كان نوع الوحدة هو "food"، يتم التحقق من التوفر بناءً على الوقت
    if (Config::get('module.current_module_type') == 'food') {
        $products = $products->available($time);
    }
    
    // جلب المنتجات الأحدث وتقسيمها على صفحات
    ...
    public function place_order(Request $request)
    {
        ...
        foreach ($cart as $c) {
            ...
            {
                $store_id = null;
                $product = Item::withoutGlobalScope(StoreScope::class)->with('product_item.store')->find($c['id']);
