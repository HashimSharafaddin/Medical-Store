...
@section('content')
    <div class="content container-fluid">
        ...
        $delivery_mans= \App\Models\DeliveryMan::get();
                    ...
                    <div class="row g-3">
                     ...
                     <div class="col-sm-6 col-md-3">
                            <select name="deliveryman_id" id="deliveryman" data-url="{{ url()->full() }}" data-filter="deliveryman_id"
                                    ...
                                    class="js-data-example-ajax-3 form-control set-filter">
                                @if (isset($deliveryman))
                                    <option value="{{ $deliveryman->id }}" selected>{{ $deliveryman->name }}</option>
                                @else
                        ...
                        <div class="col-sm-6 col-md-3">
                            <select class="form-control js-select2-custom set-filter" data-url="{{ url()->full() }}" data-filter="type" name="type">
                                <option value="all" {{ isset($type) && $type == 'all' ? 'selected' : '' }}>
                                    ...
                                    <option value='سند صرف' {{ isset($type) && $type == 'سند صرف' ? 'selected' : '' }}>
                                        {{ _('سند صرف') }}</option>
                                        <option value='سند قبض' {{ isset($type) && $type == 'سند قبض' ? 'selected' : '' }}>
                                            {{ _('سند قبض') }}</option>
...
<div class="disbursement-report mb-20">
    <div class="__card-3 rebursement-item">
        ...
        <h3 class="title text-008958">{{\App\CentralLogics\Helpers::format_currency(0)}}
        </h3>
    ...
    <div class="__card-3 rebursement-item">
        ...
        <h3 class="title text-FF7E0D">{{\App\CentralLogics\Helpers::format_currency($expense->sum('amount'))}}
        </h3>
    ...
    <div class="__card-3 rebursement-item">
        ...
        <h3 class="title text-FF5A54">{{\App\CentralLogics\Helpers::format_currency(0)}}
        </h3>
                        ...
                        <tbody id="set-rows">
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if ($exp->customer_id)
                                    @php($user =  \App\Models\User::where('id',$exp->customer_id)->first())
                                    {{ $exp->user->f_name.' '.$exp->user->l_name }}
                                    @elseif ($exp->store_id)
                                    @php($vendor =  \App\Models\Vendor::where('id',$exp->store_id)->first())

                                    {{ $vendor->f_name.' '.$vendor->l_name }}
                                    @elseif ($exp->delivery_man_id)
                                    @php($delivery_man =  \App\Models\DeliveryMan::where('id',$exp->delivery_man_id)->first())
