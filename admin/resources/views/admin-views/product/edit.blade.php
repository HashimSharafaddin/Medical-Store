                    ...
                    <div class="card shadow--card-2 border-0">
                        ...
                        <div class="card-body">
                            <div class="row g-2">
                                <div class="col-sm-12 col-lg-12">
                                    <div class="form-group mb-0">
                                        ...
                                        <select name="store_id"
                                                ...
                                                onchange="addStoreRow()">
                                            
                                            <!-- عرض المتاجر المحددة مسبقًا -->
                                            @foreach ($product->product_item as $productItem)
                                                <option value="{{ $productItem->store->id }}" data-store-name="{{ $productItem->store->name  }}" selected>
                                ...
                                @php($expiryDate = Carbon\Carbon::parse($product->expiry_date)->format('Y-m-d'))
                                <div class="col-sm-3">
                                    <div class="form-group mb-0">
                                        <label class="input-label"
                                            for="exampleFormControlInput1">{{ _('تاريخ الانتهاء') }}</label>
...
<input type="date" 
       ...
       class="form-control" 
       id="expiry_date"
       placeholder="{{ translate('messages.Ex:') }} 2025-03-01">
    </div>
</div>
                                ...
                                @endif
                                <div class="col-sm-6 col-lg-3" id="for_pharmacy">
                                    <div class="form-check mb-0 p-6">
                                        <input class="form-check-input" name="for_pharmacy"
                                            type="checkbox" value="1" id="for_pharmacy"
                                            {{ $product->user_type == 1 ? 'checked' : (isset($temp_product) && $temp_product == 1 && $product->user_type == 1 ? 'checked' : '') }}>
                                        <label class="form-check-label" for="for_pharmacy">
                                            {{ _('للصيدليلات فقط') }}
                                        </label>
                                    </div>
                                </div>
                ...
                <div class="card-body">
                    <div id="store-tables" class="mt-4"></div>
    ...
    <table class="table table-bordered" id="stores-table">
        ...
        <tbody>
            @foreach ($product->product_item as $productItem)
                <tr>
                    <td>
                        <input type="text" class="form-control" name="ids[]" value="{{ $productItem->id }}" readonly>
                    </td>
                    <td>
                        <input type="text" class="form-control" name="store_names[]" value="{{ $productItem->store->name }}" readonly>
                    </td>
                    <td>
                        <input type="text" class="form-control" name="store_ids[]" value="{{ $productItem->store->id }}" readonly>
                    </td>
                    <td>
                        <input type="number" class="form-control" name="quantities[]" value="{{ $productItem->qty }}" placeholder="Enter Quantity">
                    </td>
                </tr>
            @endforeach
        </tbody>
...
<script>
    ...
    selectedOptions.forEach(option => {
        ...
        // const storeName = option.getAttribute('data-store-name');

        // التحقق مما إذا كان السطر موجودًا بالفعل
        if (document.querySelector(`input[value="${storeId}"]`)) return;
        $.ajax({
            url: `{{url('/')}}/admin/store/get-store-name/${storeId}`,
            type: 'GET',
            success: function (response) {
                let storeName = response.store_name; // نفترض أن الاستجابة تحتوي على اسم المتجر
        // إضافة سطر جديد في الجدول
        const newRow = `
            <tr>
                <td><input type="text" class="form-control" name="ids[]" value="0" readonly></td>
                <td><input type="text" class="form-control" name="store_names[]" value="${storeName}" readonly></td>
                <td><input type="text" class="form-control" name="store_ids[]" value="${storeId}" readonly></td>
                <td><input type="number" class="form-control" name="quantities[]" placeholder="Enter Quantity"></td>
            </tr>
        `;
    ...
    $('#product_form').on('submit', function() {
        ...
        $.post({
            ...
            console.error('حدث خطأ أثناء حفظ الصنف:', xhr);
            toastr.error('حدث خطأ أثناء حفظ الصنف:', Error, {
            CloseButton: true,
            ProgressBar: true
        });
    },
