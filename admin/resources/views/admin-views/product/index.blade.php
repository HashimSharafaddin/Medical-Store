            ...
            <div class="col-md-12">
                <div class="card shadow--card-2 border-0">
                    ...
                    <div class="card-body">
                        ...
                        <div class="col-sm-12 col-lg-12"> <!-- زيادة عرض الحاوية -->
                        <div class="form-group mb-0">
                            ...
                            <div class="col-sm-3">
                                <div class="form-group mb-0">
                                    ...
                                    <input type="date" 
                                        name="expiry_date" class="form-control" id="expiry_date"
                                        placeholder="{{ translate('messages.Ex:') }} 2025/3">
                                </div>
                            </div>
                                ...
                                <div class="col-sm-6 col-lg-3" id="is_prescription_required">
                                    <div class="form-check mb-sm-2 pb-sm-1">
                                        <input class="form-check-input" name="is_prescription_required" type="checkbox"
                                            value="1" id="flexCheckDefaultprescription" >
                            ...
                            <div class="col-sm-6 col-lg-3" id="for_pharmacy">
                                <div class="form-check mb-sm-2 pb-sm-1">
                                    <input class="form-check-input" name="for_pharmacy" type="checkbox"
                                        value="1" id="flexCheckDefaultprescription" >
                                    <label class="form-check-label" for="flexCheckDefaultprescription">
                                        {{ _('للصيدليات فقط') }}
                                    </label>
...
@push('script_2')
    ...
    <script>
        ...
        $('#store_id').on('change', function () {
            let route = '{{url('/')}}/admin/store/get-addons?data[]=0&store_id=' + $(this).val()[0]; 
            let id = 'add_on';
            getRestaurantData(route, id);
            $('#store-tables').empty(); // مسح الجداول القديمة
            var selectedStores = $(this).val(); // الحصول على المتاجر المحددة

           if (selectedStores) {
    selectedStores.forEach(function (store) {
        // استدعاء AJAX لجلب اسم المتجر باستخدام معرف المتجر
        $.ajax({
            url: `{{url('/')}}/admin/store/get-store-name/${store}`,
            type: 'GET',
            success: function (response) {
                let storeName = response.store_name; // نفترض أن الاستجابة تحتوي على اسم المتجر
                
                // إضافة جدول جديد لعرض البيانات
                $('#store-tables').append(`
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td><input type="text" class="form-control" name="store_ids[]" value="${store}" readonly></td>
                                    <td><input type="number" class="form-control" name="quantities[]" placeholder="ادخل الكميه"></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
            ...
            error: function () {
                console.log('Error fetching store name.');
            }
        });
    });
}
        ...
        $('#item_form').on('submit', function (e) {
            ...
            $.post({
                ...
                success: function (data) {
                    ...
                    $('#loading').hide();
            console.error('حدث خطأ أثناء حفظ الفاتورة:', xhr);
            toastr.error('حدث خطأ أثناء حفظ الفاتورة:', error, {
            CloseButton: true,
            ProgressBar: true
        });
