                    ...
                    <h1 class="page-header-title">
                        ...
                        <span>
                            {{ translate('order_details') }} <span
                                class="badge badge-soft-dark rounded-circle ml-1">{{ $order->details->where('store_id', \App\CentralLogics\Helpers::get_store_id())->count() }}</span>
                        </span>
                    </h1>
                </div>
                       ...
                       $product_price= 0;
                        ...
                        <div class="table-responsive">
                            <table
                                ...
                                <tbody>
                                    @foreach ($order->details->where('store_id', \App\CentralLogics\Helpers::get_store_id()) as $key => $detail)
                            ...
                            <div class="mb-4">
                                <div class="row g-1">
                                    ...
                                    <input type="text" name="invoice_number" id="invoice_number"
                                        ...
                                        value="{{ $order->details->where('store_id', \App\CentralLogics\Helpers::get_store_id())->first()?->order_store_number }}" required>
...
@push('script_2')
    ...
    $.get({
        url: '{{ route('vendor.order.update-price') }}',
        dataType: 'json',
        data: {
            items: items,
            order_id: '{{ $order->id }}',
