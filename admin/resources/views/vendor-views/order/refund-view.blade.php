...
@section('content')
    ...
    <div class="content container-fluid">
            ...
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title">
                        ...
                        <span>
                            {{ _('طلب مرتجع') }} <span
                                class="badge badge-soft-dark rounded-circle ml-1">{{ $order->details->where('refund_request',1)->where('store_id', \App\CentralLogics\Helpers::get_store_id())->count() }}</span>
                        </span>
                    </h1>
                </div>
        ...
        <div class="row" id="printableArea">
                    ...
                    @endif --}}
                            ...
                            <div class="text-right mt-3 order-invoice-right-contents text-capitalize">
                                ...
                                <h6>
                                    ...
                                    @if ($order->refund['order_status'] == 'pending')
                                        <span class="badge badge-soft-info ml-2 ml-sm-3 text-capitalize">
                                            {{ translate('messages.pending') }}
                                        </span>
                                    @elseif($order->refund['order_status'] == 'confirmed')
                                        <span class="badge badge-soft-info ml-2 ml-sm-3 text-capitalize">
                                            {{ translate('messages.confirmed') }}
                                        </span>
                                    @elseif($order->refund['order_status'] == 'processing')
                                        <span class="badge badge-soft-warning ml-2 ml-sm-3 text-capitalize">
                                            {{ translate('messages.processing') }}
                                        </span>
                                    @elseif($order->refund['order_status'] == 'picked_up')
                                        <span class="badge badge-soft-warning ml-2 ml-sm-3 text-capitalize">
                                            {{ translate('messages.out_for_delivery') }}
                                        </span>
                                    @elseif($order->refund['order_status'] == 'delivered')
                                        <span class="badge badge-soft-success ml-2 ml-sm-3 text-capitalize">
                                            {{ translate('messages.delivered') }}
                                        </span>
                                    @elseif($order->refund['order_status'] == 'failed')
                                        <span class="badge badge-soft-danger ml-2 ml-sm-3 text-capitalize">
                                            {{ translate('messages.payment_failed') }}
                                        </span>
                                    @else
                                        <span class="badge badge-soft-danger ml-2 ml-sm-3 text-capitalize">
                                            {{ str_replace('_', ' ', $order->refund['order_status']) }}
                                        </span>
                                    @endif
                                </h6>
                            ...
                            <table
                                ...
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th class="border-0">{{ translate('messages.expiry_date') }}</th>
                                        ...
                                        @endif
                                        {{-- <th class="text-right  border-0">{{ _('تاكيد') }}</th> --}}

                                        <th class="text-right  border-0">{{ translate('messages.total_price') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($order->details->where('refund_request',1)->where('store_id', \App\CentralLogics\Helpers::get_store_id()) as $key => $detail)
                                        @if (isset($detail->item_id))
                                            ...
                                            @php($product_pricep += $detail->price * $detail->quantity)
                                            ...
                                            <tr>
                                                ...
                                                <td>{{$detail->item['expiry_date']}}</td>
                                                <td>{{ $detail['refund_qty'] }}</td>

                                                <td>     <input type="number"
                                                    id="item_price_{{ $detail['id'] }}"
                                                    data-id="{{ $detail['id'] }}"
                                                    order-id="{{ $order['id'] }}"
                                                    name="item_price"
                                                    class="form-control item-price-input"
                                                    value="{{ $detail['price'] }}" readonly></td>
                        ...
                        <div class="row justify-content-md-end mb-3 mx-0 mt-4">
                            <div class="col-md-9 col-lg-8">
                                <dl class="row text-right">
                                    ...
                                    <dt class="col-6">{{ translate('messages.total') }}
                                    ...
                                    <dd class="col-6">
                                        {{ \App\CentralLogics\Helpers::format_currency($product_pricep + $total_tax_amount + $total_addon_price + $additional_charge - $coupon_discount_amount - $store_discount_amount - $admin_flash_discount_amount  - $ref_bonus_amount + $extra_packaging_amount-$store_flash_discount_amount + $order->dm_tips) }}
                                    </dd> --}}
                                    @if ($order?->payments)
            ...
            <div class="col-lg-4">
                <!-- Card -->
                <div style="background-color: #c1f4a4" class="card-header justify-content-center text-center px-0 mx-4">
                    <h5 class="card-header-title text-capitalize">
                        <span>{{ translate( $order->refund->order_status) }}</span>
                    </h5>
                </div>
                ...
                @if ($order->refund->order_status != 'refund_requested' &&
                    $order->refund->order_status != 'refunded' &&
                    $order->refund->order_status != 'delivered')
                    <div class="card mb-2">
                        <!-- Header -->
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-4">
                                <div class="row g-1">
                                    ...
                                    @else
                                    <a href="#" class="btn btn--primary w-100 save-order">
                                        تاكيد وحفظ
                                    </a>
                                    
                                    @endif
                                    <input type="text" name="invoice_number" id="invoice_number"
                                        class="form-control mt-2 invoice_number" placeholder="{{ _(' ادخل رقم الفاتورة ') }}"
                                        value="{{ $order->details->where('store_id', \App\CentralLogics\Helpers::get_store_id())->first()?->refund_store_number }}" required>
                ...
                <ul class="delivery--information-single mt-3">
                    ...
                    <li>
                        <span class="name">{{ translate('Cancel_Reason') }} </span>
                        <span class="info">  {{ $order->refund->customer_reason ?? translate('messages.N/A')}} </span>
                    </li>
                    <li>
                        <span class="name">{{ translate('Cancel_Note') }} </span>
                        <span class="info">  {{ translate($order->refund->customer_note) }} </span>
                    </li>
                            ...
                            @if ( $order?->payments)
                                ...
                                @foreach ($pay_infos as $pay_info)
                                    <li>
                                        ...
                                        <span class="info">  {{ \App\CentralLogics\Helpers::format_currency($order->refund->refund_amount)  }} </span>
                                    </li>
                                @endforeach
                            @else
                            <li>
                    ...
                    @if ($order->payment_status == 'paid' || $order->payment_status == 'partially_paid')
                            @php( $amount =$order->payments()->where('payment_status','paid')->sum('amount'))
                                <li>
                                    <span class="name">{{ translate('Amount_Returned') }} </span>
                                    <span class="info">  {{ \App\CentralLogics\Helpers::format_currency($order->refund->refund_amount)  }} </span>
                                </li>
                    ...
                    {{-- @if (!$order->customer) --}}
                        <div class="card-body">
                            ...
                            <div class="media align-items-center customer--information-single" href="javascript:">
                                ...
                                <div class="media-body">
                                   <span class="text--title font-semibold d-flex align-items-center">
                                        <i class="tio-id-talking-quiet mr-2"></i> رقم العميل
                                        {{ $order->customer['id'] }}
                                    </span>
                                    <span class="text--title font-semibold d-flex align-items-center">
                                        <i class="tio-shopping-basket-outlined mr-2"></i>
...
@push('script_2')
    ...
    <script type="text/javascript">
        "use strict";
        const bootstrapCSS = "{{ asset('public/assets/installation/assets/css/bootstrap.min.css') }}";
        const styleCSS = "{{ asset('public/assets/installation/assets/css/style.css') }}";
    ...
    if (invoiceNumber === '') {
        toastr.error("ادخل رقم الفاتورة الخاصه بك", {
            CloseButton: true,
            ProgressBar: true
        });
        return;
    ...
    $.get({
        url: '{{ route('vendor.order.refund-update-price') }}',
        dataType: 'json',
        data: {
            items: items,
            order_id: '{{ $order->id }}',
        ...
        function printCard() {
    const cardBody = document.querySelector('.col-lg-8.mb-3.mb-lg-0');
