...
<head>
    ...
    <link rel="shortcut icon" href="{{ asset('public/icons/store_logo.png') }}">
    <link rel="icon" type="image/x-icon" href="{{ asset('public/icons/store_logo.png') }}">
    ...
    <link rel="apple-touch-icon" href="{{ asset('public/icons/store_logo.png') }}">
    {{-- <meta name="theme-color" content="#ffffff"> <!-- اختيار لون الخلفية --> --}}
    <!-- Font -->
...
<!-- End Footer -->
                        ...
                        <div class="col-12">
                            <div class="text-center">
                                ...
                                <a class="nav-link" href="{{route('admin.order.list',['all'])}}" title="{{translate('messages.all_orders')}}">
                                    ...
                                    <button href="{{route('admin.order.list',['all'])}}"   class="btn btn-primary check-order">{{translate('messages.Ok, let me check')}}</button>
                                    </a>                            </div>
                        </div>
                    </div>
                </div>
    ...
    @php($users = \App\Models\User::where('status', 0)->get())
    {{-- @foreach($users as $user)
    <!-- Modal لكل مستخدم -->
    <div class="modal fade" id="activateUserModal-{{$user->id}}" tabindex="-1" aria-labelledby="activateModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <form action="{{ route('admin.users.customer.status', [$user->id, $user->status ? 0 : 1]) }}" method="GET">
            @csrf
            @method('PUT')
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">تفعيل حساب المستخدم</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="إغلاق"></button>
                </div>
                <div class="modal-body">
                    هل أنت متأكد أنك تريد تفعيل الحساب للمستخدم: <strong>{{ $user->name }}</strong>؟
                </div>
...
setInterval(function () {
    $.get({
        url: '{{ route('admin.order.get-order-data') }}',
        dataType: 'json',
        success: function (response) {
            ...
            if (data.new_user && data.new_user.id > 0) {
    playAudio();
    let status = data.new_user.status ? 0 : 1;
    let urlTemplate2 = '{{ route("admin.users.customer.view", ["user_id" => "USER_ID"]) }}';
let url1 = urlTemplate2
    .replace("USER_ID", data.new_user.id)
let urlTemplate = '{{ route("admin.users.customer.status", ["customer" => "USER_ID", "status" => "USER_STATUS"]) }}';
let url = urlTemplate
    .replace("USER_ID", data.new_user.id)
    .replace("USER_STATUS", status);
  

    if (!$('#activateUserModal-' + data.new_user.id).length) {
        $('body').append(`
            <div class="modal fade" id="activateUserModal-${data.new_user.id}" tabindex="-1" aria-labelledby="activateModalLabel" aria-hidden="true">
              <div class="modal-dialog">
                <form action="${url}" method="GET">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">طلبات المستخدمين</h5>
                        </div>
                        <div class="modal-body">
                             <div class="form-group">
                            <label for="name">معلومات الطلب</label>
                              
                            <strong>${data.new_user.note}</strong>؟

                            </div>
                           <div class="form-group">
                            <label for="name">اسم المستخدم</label>
                              
                            <strong>${data.new_user.f_name}  ${data.new_user.l_name}</strong>؟

                            </div>
                        </div>
                        <div class="modal-footer">
                             
                            <button type="button" class="btn btn-info" data-bs-dismiss="modal fade"><a href="${url1}" target="_blank">
                                         عرض   </a></button>
    ...
    $('#activateUserModal-' + data.new_user.id).modal('show');
}
