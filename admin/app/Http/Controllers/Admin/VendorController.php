...
// $idS = [2,3];
        ...
        $store->logo = Helpers::upload('store/', 'png', $request->file('logo'))??"store_logo.png";
        $store->cover_photo = Helpers::upload('store/cover/', 'png', $request->file('cover_photo'))??"store_logo.png";
        ...
        // $store->modules()->attach($idS); // يجب أن يكون `module_ids` مصفوفة تحتوي على IDs للوحدات
...
//             $store1 = new Store;
//             $store1->name = $request->name[array_search('default', $request->lang)];
//             $store1->phone = $request->phone;
//             $store1->email = $request->email;
//             $store1->logo = Helpers::upload('store/', 'png', $request->file('logo'))??"store_logo.png";
//             $store1->cover_photo = Helpers::upload('store/cover/', 'png', $request->file('cover_photo'))??"store_logo.png";
//             $store1->address = $request->address[array_search('default', $request->lang)];
//             $store1->latitude = $request->latitude;
//             $store1->longitude = $request->longitude;
//             $store1->vendor_id = $vendor->id;
//             $store1->zone_id = $request->zone_id;
//             $store1->tax = $request->tax??0;
//             $store1->delivery_time = $request->minimum_delivery_time .'-'. $request->maximum_delivery_time.' '.$request->delivery_time_type;
//             $store1->module_id = 3;

//             $store1->save();
// $store1->module->increment('stores_count');
    ...
    public function get_store_name($id)
    {
        $store = Store::find($id);
    if ($store) {
        return response()->json(['store_name' => $store->name]);
    }
