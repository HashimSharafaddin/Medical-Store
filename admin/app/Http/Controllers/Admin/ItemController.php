...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'expiry_date.required' => translate('messages.expiry date is required'),
        ...
        if (isset($request->store_ids) && is_array($request->store_ids) && count($request->store_ids) === 1) {
            $store_id = $request->store_ids[0]??0;
        }
        
        $item->food_variations = json_encode($food_variations);
        $item->variations = json_encode($variations);
        $item->price = $request->price;
        $item->image =  $request->has('image') ? Helpers::upload('product/', 'png', $request->file('image')) : $newFileNamethumb??'' ;
        ...
        $item->user_type = $request->for_pharmacy??0;
        $item->company_name = $request->generic_name??"";
        ...
        foreach ($request->store_ids as $index => $id) {
            ...
            // الوصول إلى الكمية والسعر باستخدام الفهرس بدلاً من معرف المتجر
            $product->qty = $request->quantities[$index];
        ...
        $item->stock = $qty ;
        $item->store_ids = json_encode($request->store_ids);
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($request->temp_product) {
            $product = TempProduct::withoutGlobalScope(StoreScope::class)->withoutGlobalScope('translate')->with('category', 'module', 'product_item.store')->findOrFail($id);
            $temp_product = true;
        } else {
            $product = Item::withoutGlobalScope(StoreScope::class)->withoutGlobalScope('translate')->with('category', 'module', 'product_item.store')->findOrFail($id);
        }
        if (!$product) {
        ...
        $allStores = Store::all(); // جلب جميع المتاجر

        return view('admin-views.product.edit', compact('product', 'sub_category', 'category', 'temp_product', 'allStores'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($request->store_ids as $index => $storeId) {
            ...
            // تحقق مما إذا كان السطر الجديد أم تحديث
            if ($request->ids[$index] == 0) {
                // إنشاء عنصر جديد إذا كانت القيمة 0
                $product = new ProductItem();
            } else {
                // البحث عن العنصر إذا كانت القيمة موجودة
                $product = ProductItem::find($request->ids[$index]);
        
                // إذا لم يتم العثور على السجل، إنشاء عنصر جديد
                if (!$product) {
                    $product = new ProductItem();
                }
            }
            ...
            // الوصول إلى الكمية والسعر باستخدام الفهرس
            $product->qty = $request->quantities[$index];
            $product->price = $item->price; // تعيين السعر من الطلب بدلاً من `$item->price`
        $qty += $request->quantities[$index];
            // حفظ المنتج في قاعدة البيانات
            $product->save();
        }
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        if ($storeId==null) {
            Toastr::error(_('الرجاء اختيار المورد') );
            return back();
        }
        ...
        if ($request->button == 'import') {
            ...
            try {
                foreach ($collections as $collection) {
                    ...
                    //     return back();
                    // }
                    DB::table('items')->where("store_id", $storeId)->delete();
                    $unit = Unit::where('unit', $collection['UnitId'])->first();
                    if (!$unit) {
                        $newUnit = Unit::create(['unit' => $collection['UnitId']]);
                        $unit_id = $newUnit->id;
                    } else {
                        $unit_id = $unit->id;
                    }
                    array_push($data, [
                        ...
                        'category_id' => 1,
                        'category_ids' => '{"id":0,"position":1}]',
                        'unit_id' => $unit_id ??null,
                        'stock' => is_numeric($collection['Stock']) ? abs($collection['Stock']) : 1000,
                        'price' => isset($collection['Price']) && is_numeric($collection['Price']) ? $collection['Price'] : 1000, // معالجة السعر
                        'discount' => 0,
                        ...
                        'store_id' => $storeId,
                        'store_ids' => json_encode([]),
                        'expiry_date' => $collection['expiry_date']??"2027-01-01 00:00:00",
                    'user_type' =>0,
                        'module_id' => $module_id,
                        'status' => 0,
            ...
            try {
                ...
                $chunkSize = 100;
                $chunkItems = array_chunk($data, $chunkSize);
                ...
                foreach ($chunkItems as $chunk) {
                    foreach ($chunk as $item) {
                        ...
                        // $productItems = [];
                        // $storeIds = json_decode($item['store_ids'], true);
                        
                        // Ensure $storeIds is an array and not empty
                        // if (is_array($storeIds) && count($storeIds) > 0) {
                        //     $qty = $item['Stock'] / count($storeIds); // Distribute stock equally
                        
                        //     foreach ($storeIds as $storeId) {
                        //         $productItems[] = [
                        //             'store_id' => $storeId,
                        //             'item_id' => $insertedId,
                        //             'qty' => round($qty, 2), // Round the quantity for precision
                        //             'price' => $item['price'],
                        //         ];
                        //     }
                        ...
                        // } else {
                        //     // Handle the case where store_ids is invalid
                        //     info("Invalid store_ids for item_id: {$insertedId}");
                        //     Toastr::error(translate('messages.invalid_store_ids'));
                        //     return back();
                        // }
                    }
                }
                ...
                DB::rollBack();
                info(["line___{$e->getLine()}", $e->getMessage(), $e->getTraceAsString()]);
                Toastr::error($e->getMessage());
                return back();
            }
        ...
        //             'store_ids' => $collection['StoreIds'],
        //             'expiry_date' => $collection['expiry_date']??"2027-01-01 00:00:00",
        //             'user_type' => $collection['userType'],
        //             'module_id' => $module_id,
