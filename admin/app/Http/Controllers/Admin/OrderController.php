...
class OrderController extends Controller
{
    public function list($status, Request $request)
    {
        ...
        public function list_refund(Request $request)
        {
            ...
            return view('admin-views.order.list-refund', compact('orders', 'status'));
        }
        public function refund_details($id)
        {
            ...
            return view('admin-views.order.refund-view', compact('order','reasons'));
        }
    ...
    public function status(Request $request)
    {
        ...
        if ($request->order_status == 'confirmed') {
            ...
            foreach ($order->details as $details) {
                ...
                $detail = OrderDetail::where('id',$details->id)->first();
                if($detail )
    {
        $detail->success = 1;
        $detail->save();
            ...
            if ($order->transaction  == null) {
                ...
                if ($order->store_id == 0) {
                    foreach ($order->details as $details) {
                        ...
                        $comission = isset($store->comission) == null?\App\Models\BusinessSetting::where('key','admin_commission')->first()->value:$order->store->comission;
                        $comission_on_store_amount = ($comission?($details->price/ 100) * $comission:0);
                        $amount =$details->price - $comission_on_store_amount;
                        if ($store && $store->vendor && $store->vendor->wallet) {
                            $store->vendor->wallet->total_earning +=  $amount;
    ...
    public function updateQuantity(Request $request)
{
    ...
    $detail->save();
    $order_amount=0;
    foreach($order->details as $detail1){
        $order_amount += $detail1->price * $detail1->quantity;
    }
    ...
    public function getOrderData(): JsonResponse
    {
        ...
        $new_user = User::where('status', 0)->where('new_user', 0)->first();

        return response()->json([
            'success' => 1,
            'data' => ['new_order' => $newOrder, 'new_user' => $new_user]
        ]);
    }
    ...
    {
        $item = OrderDetail::findOrFail($request->id);
