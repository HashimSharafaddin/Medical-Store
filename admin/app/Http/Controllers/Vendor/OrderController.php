...
class OrderController extends Controller
{
  ...
  ->StoreOrder()->NotDigitalOrder()
        ->when($store_id, function ($query) use ($store_id) {
            $query->whereJsonContains('store_ids', \App\CentralLogics\Helpers::get_store_id());
        })
        // ->StoreOrder()->NotDigitalOrder()
        // ->where('store_ids',)
    ...
    public function list_refund(Request $request)
    {
        ...
        $orders = Order::with(['customer', 'refund', 'details'])
        ->whereHas('refund') // ← هنا الفرق
        ...
        ->where('cutlery', 0)
        ->when($store_id, function ($query) use ($store_id) {
            $query->whereJsonContains('store_ids', $store_id);
        })
        ->orderBy('schedule_at', 'desc')
        ->paginate(config('default_pagination'));
    
    
        return view('vendor-views.order.list-refund', compact('orders', 'status'));
    }
    public function refund_details($id)
    {
        $reasons=OrderCancelReason::where('status', 1)->where('user_type' ,'store' )->get();

        $order = Order::with(['customer', 'refund', 'details'])->where(['id' => $id])->first();
        return view('vendor-views.order.refund-view', compact('order','reasons'));
    }
    public function getOrderData(): JsonResponse
    {
        $newOrder = Order::where('order_status','accepted')->count();
    ...
    public function export_orders($file_type, $status, $type, Request $request)
    {
        ...
        ->when(in_array($status, ['pending','confirmed','accepted']), function($query){
            return $query->OrderScheduledIn(30);
        })
        ->when(isset($key), function ($query) use ($key) {
            return $query->where(function ($q) use ($key) {
...
public function updateQty(Request $request)
{
    ...
    $detail = OrderDetail::where('id', $request->id)
    ->first();
    ...
    foreach($order->details as $detail){
        $order_amount += $detail->price * $detail->quantity;
    }
    $order->amounorder_amount = $order_amount;
    $order->save();
...
public function updatePrice(Request $request)
{
    ...
    // الحالات التي لا يُسمح بالتعديل فيها
    $lockedStatuses = ['delivered', 'out_for_delivery'];

    if (in_array($order->order_status, $lockedStatuses)) {
        return response()->json(['message' => 'لا يمكن تعديل السعر بعد التوصيل أو التسليم'], 403);
    }
    $deliveryman = DeliveryMan::where('id', $order->delivery_man_id)->available()->active()->first();
    $store = Store::where('id', Helpers::get_store_id())->first();

    foreach ($request->items as $item) {
        $detail = OrderDetail::where('store_id', Helpers::get_store_id())
            ->where('id', $item['id'])
            ->first();

        if ($detail) {
            $detail->price = $item['price'];
            $detail->order_store_number = $request->invoice_number;
            $detail->delivery_status = 'picked_up';
            $detail->success = 1;
            ...
            $detail->save();
                    ...
                    $data = [
                        ...
                        'description' => 'الطلب جاهز للاستلام من ' . $store->name,
                        'order_id' => $order['id'],
                        'image' => '',
                        'type' => 'order_status'
                    ];
    ...
    if($order->details->sum('success') == $order->details->count()){
...
public function refundUpdatePrice(Request $request)
{
    $validator = Validator::make($request->all(), [
        'items' => 'required|array|min:1',
        'items.*.id' => 'required|exists:order_details,id',
    ...
    $deliveryman = DeliveryMan::where('id', $order->delivery_man_id)->available()->first();
$store = Store::where('id', Helpers::get_store_id())->first();
    foreach ($request->items as $item) {
        $detail = OrderDetail::where('store_id', Helpers::get_store_id())
            ->where('id', $item['id'])
        ...
        if ($detail) {
            // $detail->price = $item['price'];
            $detail->refund_store_number = $request->invoice_number;
            ...
            $detail->save();
                    ...
                    $data = [
                        'title' => translate('Order_Notification'),
                        'description' =>'تم استلام المرتجع من المورد  ' . $store->name,
                        'order_id' => $order['id'],
                        'image' => '',
                        'type' => 'order_status'
                    ];
    ...
    if($order->details->where('refund_request',1)->where('refund_store_number','!=',null)->count() == $order->details->where('refund_store_number','!=',null)->count()){
        $order->refund->update([
    ...
    public function details(Request $request,$id)
    {
        ...
        $order = Order::with(['details','offline_payments','customer'=>function($query){
            ...
            return $query->withCount('orders');
        }])->where(['id' => $id])->StoreOrder()->NotDigitalOrder()
        ->when($store_id, function ($query) use ($store_id) {
            $query->whereJsonContains('store_ids', \App\CentralLogics\Helpers::get_store_id());
        })->first();
        ...
        // } else {
        //     Toastr::info('لايمكن عرض الطلب بعد التوصيل!');
        //     return back();
        // }
    }
    public function orderData(): JsonResponse
    {
        $newOrder = OrderDetail::where('store_id',Helpers::get_store_id())->where('checked',0)->count();
    ...
    public function status(Request $request)
    {
        ...
        $order = Order::when('details')->where(['id' => $request->id])->first();
        $newOrder = OrderDetail::where('store_id',Helpers::get_store_id())->get();
        ...
        foreach ($order->details as $details) {
            // تحميل المتجر مع علاقات vendor و wallet
            $detail = OrderDetail::where('store_id',Helpers::get_store_id())->where('id',$details->id)->first();
            if($detail )
{
    $detail->success = 1;
    $detail->save();
            ...
            if($order->transaction  == null)
            {
                ...
                if ($order->store_id == 0) {
                    foreach ($order->details as $details) {
                        ...
                        $detail->success = 1;
                        $detail->save();
                        $store = Store::with(['vendor.wallet'])->where('id', $details->store_id)->first();
                        if ( !$store->vendor->wallet) {
                            $wallet = new \App\Models\StoreWallet();
                            $wallet->vendor_id = $store->vendor->id;
                            $wallet->total_withdrawn = $details->price;
                            $wallet->save();
                        }
                        // تحقق مما إذا كان البائع والمحفظة موجودين
                        if ($store && $store->vendor && $store->vendor->wallet) {
                            $store->vendor->wallet->total_earning += $details->price;
                            $store->vendor->wallet->save(); // حفظ التغييرات في المحفظة
                        } else {
                            // في حالة عدم وجود محفظة، يمكنك إنشاء محفظة جديدة للبائع
                            if ($store && $store->vendor && $store->vendor->wallet) {
                                $wallet = new \App\Models\StoreWallet();
                                $wallet->vendor_id = $store->vendor->id;
                                $wallet->total_withdrawn = $details->price;
    ...
    public function itemSuccess($id)
    {

        $order_d = OrderDetail::where(['id' => $id])->first();
        $order = Order::when('details')->where(['id' => $order_d->order_id])->get();

        if($order_d->success == 0){
            $order_d->success =1 ;       
        }else{
            $order_d->success =0 ;       
