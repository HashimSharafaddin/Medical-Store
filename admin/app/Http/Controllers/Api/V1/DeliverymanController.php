...
class DeliverymanController extends Controller
{
    ...
    public function readyStatus(Request $request)
    {
        $dm = DeliveryMan::with(['rating'])->where(['auth_token' => $request['token']])->first();
        $dm->ready = $dm->ready?0:1;
        $dm->save();
        return response()->json(['message' => translate('messages.active_status_updated')], 200);
    }
    ...
    public function accept_order_item(Request $request)
    {
        ...
        try {
            // if ($request->order_status == 'confirmed') {
                $order = Order::when('details')->where('id',  $orderId??$request->order_id )->first();
                ...
                foreach ($order->details as $details) {
                    ...
                    $detail = \App\Models\OrderDetail::where('id',$details->id)->where('store_id',$storeId??$request->store_id)->first();
                    if($detail )
        {
            $detail->success = 1;
            $detail->delevery_status = 'picked_up';
            $detail->save();
        }
    ...
    foreach ($order->details as $details) {
        $detail = \App\Models\OrderDetail::where('id',$details->id)->first();
        $cuont1++;
        if($detail->success==1){
            $cuont2++;
        }
    ...
    if($cuont1==$cuont2){
        ...
        if(Helpers::getNotificationStatusData('deliveryman','deliveryman_order_assign_unassign','push_notification_status')){
            $data = [
                'title' => translate('Order_Notification'),
                'description' => translate('messages.out_for_delivery'),
                'order_id' => $order['id'],
                'image' => '',
                'type' => 'order_status'
            ];
            // Helpers::send_push_notif_to_device($order->customer->fcm_token, $data);
            DB::table('user_notifications')->insert([
                'data' => json_encode($data),
                'user_id' => $order->customer->id??$order->user_id,
                'created_at' => now(),
                'updated_at' => now()
            ]);
        }
...
return response()->json(['message' => $e->getMessage()], 500);
    ...
    public function update_order_refund_status(Request $request)
{
        $validator = Validator::make($request->all(), [
            'order_id' => 'required',
            'status' => 'required|in:refund_requested,refunded',
        ]);
        if ($validator->fails()) {
            return response()->json(['errors' => Helpers::error_processor($validator)], 403);
        }
        $dm = DeliveryMan::where(['auth_token' => $request['token']])->first();
        $order = Order::with(['customer', 'store','refund'])-> where(['id' => $request['order_id']])->first();    
        ...
        //     'updated_at' => now()
        // ]);
        if($order->refund->refund_status == "pending"){
            $order->refund->update([
