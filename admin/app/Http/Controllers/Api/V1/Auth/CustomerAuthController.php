...
class CustomerAuthController extends Controller
{
    ...
    }    public function uplodeImag(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'image' => 'required',
...
try{
       ...
       $imageName = $request->has('image') ? Helpers::upload('profile/', 'jpg', $request->file('image')) : $newFileNamethumb ?? null;
}catch(\Exception $e){
    return response()->json([
        'message' => translate('messages.failed')
    ], 200);
...
'pharmcy_name'=> $request->pharmcy_name??null,
        ...
        //         'errors' => $errors
        //     ], 403);
        if($request->userType=="pharmacy"){
            $address = [
                'user_id' => $user->id,
                'contact_person_name' => $request->name,
                'contact_person_number' => $request->phone,
                'address_type' => 'العنوان',
                'address' => $request->address,
                'floor' => $request->floor??'',
                'road' => $request->address,
                'house' => $request->house??'',
        ...
        if(isset($login_settings['phone_verification_status']) && $login_settings['phone_verification_status'] == 1){
            ...
            if(!$firebase_otp_verification){
                ...
                if($published_status == 1){
                    $response = \App\Traits\SmsGateway::send($request['phone'],$otp);
                }else{
