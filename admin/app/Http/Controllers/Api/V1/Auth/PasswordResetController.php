...
class PasswordResetController extends Controller
{
    public function reset_password_request(Request $request)
    {
        ...
        if (isset($customer)) {
            ...
            if($published_status == 1){
                $response = \App\Traits\SmsGateway::send($request['phone'],$token);
            }else{
    ...
    public function verify_token(Request $request)
    {
        ...
        if ( bcrypt($request->old_password, ) != bcrypt($user->password)) {
            return response()->json(['error' => 'كلمة المرور القديمة غير صحيحة'], 422);
        }
