...
class OrderController extends Controller
{
    ...
    $digital_payment = Helpers::get_business_settings('digital_payment');
        ...
        $textSpeeder = '';
        if($request->speeder== 1){
            $delivery_charge +=(float)BusinessSetting::where(['key' => 'fastـdelivery'])->first()->value;
            //  (float)BusinessSetting::where(['key' => 'fastـdelivery'])->first()->value;
            
            $textSpeeder = 'توصيل مستعجل';

        }
        // if($request->order_types!= null){
        //     $order_types = Helpers::get_business_settings('order_types');
        // if (isset($apples)) {
            // foreach (Helpers::get_business_settings('order_types') as $type) {
            //     if($type['name'] == $request->order_types){
            //         $delivery_charge +=$type['amount'];
            //     };
                // $config1 = [
                //     'name' => $order_type['name'],
                //     'status' => (boolean)$order_type['status'],
                //     'amount' => $order_type['amount'],
                // ];
                // array_push($order_types, $config1);
            // }
        // }
            // $delivery_charge +=(float)BusinessSetting::where(['key' => 'order_types'])->first()->value;
            //  (float)BusinessSetting::where(['key' => 'fastـdelivery'])->first()->value;
...
//            'contact_person_number' => $request->contact_person_number ? ($request->user ? $request->contact_person_number :str_replace('+', '', $request->contact_person_number)) : ($request->user?$request->user->phone:''),
        ...
        $order->delivery_instruction = $request['delivery_instruction']." "." : ". $textSpeeder .' التوصيل عبر' .':'.$request['dType'] ;
        $order->order_type = $request['order_type'];
        $order->order_types = $request['order_types'];
        ...
        $carts = Cart::where('user_id', $order->user_id)->where('to_cart',1)->where('is_guest',$order->is_guest)->where('module_id',$request->header('moduleId'))
            ...
            foreach ($carts as $c) {
                if ($c['item_type'] === 'App\Models\ItemCampaign' || $c['item_type'] === 'AppModelsItemCampaign') {
                    ...
                    if ($product) {
                        ...
                        //     // تحقق إذا كانت الكمية المطلوبة متوفرة
                        //     if ($item->qty >= $c['quantity']) {
                        //         $isItem = \App\Models\ProductItem::find($item->id);
                        //         if ($isItem) {
                        //             // تحديث الكمية المتاحة
                        //             $isItem->qty -= $c['quantity'];
                        //             $isItem->save(); // حفظ التغييرات في قاعدة البيانات
                        //             $store_id = $isItem->store_id;
    
                        //         }                            // إيقاف الحلقة إذا وجدت الكمية المناسبة
                        //         break;
                        //     }
                        // }
                    ...
                    $product = Item::with('module','product_item.store')->active()->find($c['item_id']);
                    if ($product) {
                            ...
                            foreach ($product->product_item as $item) {
                                ...
                                if ($item->qty >= $c['quantity']) {
                                    ...
                                    if ($isItem) {
                                        ...
                                        $isItem->save(); // حفظ التغييرات في قاعدة البيانات
                                        $store_id = $isItem->store_id;
                                        $store_ids []= $isItem->store_id;
                        // $this ->place_order($item->id,$isItem->store_id);
                                    }                            // إيقاف الحلقة إذا وجدت الكمية المناسبة
                                    break;
                                }
                            }
                       ...
                       {
                            $store_name = Store::find($product->store_id);
                        
                            $store_id = $product->store_id;
                            $store_ids []= $product->store_id;
                             $order->store_id = $product->from_user_app;
                             $order->cutlery = $product->from_user_app??0;
                        ...
                        if ($product->module->module_type == 'food' && $product->food_variations) {
                            ...
                            $or_d = [
                                ...
                                'variant' => json_encode($c['variant']),
                                'variation' => json_encode($variations),
                                'store_id' => $store_id??$product->store->id,
                                'store_name' => $store_name->name??$product->store->name,
        ...
        try {
            ...
            $order->save();
            if ($request->order_type !== 'parcel') {
                $order->store_ids = json_encode($store_ids);
            ...
            DB::commit();
            // $delivery_mans = DeliveryMan::where('active', 1)->where('ready', 1)
            // ->with(['wallet', 'vehicle'])
            // // ->where('vehicle_id',$vehicle_id) 
            // ->orderBy('order_count', 'asc') // ترتيب حسب أقل عدد في order_count
            // ->get();

            $delivery_mans = DeliveryMan::where('id', 3)->where('ready', 1)
            ->with(['wallet', 'vehicle'])->get();
            // ->where('vehicle_id',$vehicle_id) 
            // ->orderBy('order_count', 'asc') // ترتيب حسب أقل عدد في order_count
            // ->get();

        // جلب العلاقات المرتبطة
            // ->whereHas('vehicle', function($query) use ($request) {
            //     $query->where('type', $request->d_type); // شرط على نوع المركبة
            // })
        // جلب جميع البائعين دفعة واحدة
// $vendors = Vendor::whereIn('id', $store_ids)->get();

// if ($vendors->isNotEmpty()) {
//     $details = [
//         'title' => 'New Notification',
//         'message' => 'You have a new message!',
//         'line' => 'Click below to view more details.',
//         'actionText' => 'View Notification',
//         'actionURL' => url('vendor/order/list/confirmed'),
//     ];

//     // إرسال الإشعارات للمجموعة دفعة واحدة
//     Notification::send($vendors, new NotificationName($details));
// } else {
//     // في حالة عدم وجود أي بائعين
//     \Log::info('No vendors found for the given store IDs.');
// }
$data = [
    'title' => 'تاكيد الطلب',
    'description' => 'الرجاء تاكيد الطلب ولا سوف يلغاء بعد 15 دقيقة',
    'order_id' => $order['id'],
    'image' => '',
    'type' => 'block'
];
...
DB::table('user_notifications')->insert([
    ...
    'updated_at' => now()
            ...
            foreach ($delivery_mans as $deliveryMan) {
                ...
                $this->accept_order($order->id,  $deliveryMan->id,true);
            
                        break; // إيقاف الحلقة إذا تم إسناد الطلب بنجاح
                }
            }
            ...
            try {
                ...
                if(!in_array($order->payment_method, ['digital_payment', 'partial_payment', 'offline_payment'])  || $payments){
                        ...
                        // $delivery_mani = $this->accept_order($order->id,$delivery_man_id);
    ...
    private function accept_order($order_id,$delivery_man_id,bool $fromUser = false)
    {
        if ($delivery_man_id == 0) {
            return response()->json(['message'=> translate('messages.deliveryman_not_found')  ], 400);
        }
        $order = Order::withOutGlobalScope(\App\Scopes\ZoneScope::class)->find($order_id);
    ...
    public function prescription_place_order(Request $request)
    {
        ...
        $order->zone_id = isset($zone) ? $zone->id : end(json_decode($request->header('zoneId'), true));
        $order->module_id = 5;
        //  $request->header('moduleId');
        $order->pending = now();
        if(isset($images)){
            $order->order_attachment = json_encode($images);
        }
        $order->distance = $request->distance;
        $order->delivery_instruction = $request['delivery_instruction' ]?? $textSpeeder;
        $order->dm_vehicle_id = $vehicle_id;
    ...
    public function get_order_list(Request $request)
    {
        ...
        $paginator = Order::with(['store', 'delivery_man.rating', 'parcel_category', 'refund'])->withCount('details')->where(['user_id' => $user_id])->whereIn('order_status', ['delivered', 'canceled', 'refund_requested', 'refund_request_canceled', 'refunded', 'failed'])
    ...
    public function get_running_orders(Request $request)
    {
        ...
        $paginator = Order::with(['store', 'delivery_man.rating', 'parcel_category','refund'])
        // ->when(!isset($request->user) , function($query){
        //     $query->where('is_guest' , 1);
        // })
        ...
        $massage = false;
        // $order = Order::whhere(['user_id' => $user_id])->whereIn('order_status', ['pending', 'accepted', 'confirmed'])->where('created_at', '>=', now()->subSeconds(15))->first();    
        // foreach ($orders as $order) {
        //     if (
        //         in_array($order->order_status, ['pending', 'accepted', 'confirmed']) &&
        //         $order->created_at >= now()->subSeconds(15)
        //     ) {
        ...
        if ($massage) {
            $data['message'] = translate('messages.order_status_updated_successfully');
        }
   ...
   public function cancel_order(Request $request)
    {
        ...
        // $orderDetails = OrderDetail::where(['id' => $request['idz']])->get();
    ...
    public function accepted_order(Request $request)
    {
        ...
        if (!$order) {
        ...
        } else if ( $order->order_status == 'accepted'|| $order->order_status == 'confirmed') {
            ...
            // $order?->store ?   Helpers::increment_order_count($order?->store) : '';
            $data = [
                'title' => 'طلبك قيـــد المــــراجعــــــة ',
                'description' => 'طلبك قيـــد المــــراجعــــــة  وسيتم التواصل معك ', 
        ...
        return response()->json([
            'errors' => [
                ['code' => 'order', 'message' => translate('messages.you_can_not_accepted_after_confirm')]
            ]
        ], 403);
    }
    public function cancel_order1(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'reason'   => 'required|max:255',
            'guest_id' => $request->user ? 'nullable' : 'required',
            'idz'      => 'required' // التحقق من أن القائمة موجودة
        ]);
        ...
        // تحويل `idz` إلى مصفوفة باستخدام explode
        $orderIds = explode(',', $request->idz);
    
        // جلب المنتجات التي سيتم إرجاعها
        $orderDetails = OrderDetail::whereIn('id', $orderIds)->get();
    
        if ($orderDetails->isEmpty()) {
            return response()->json(['error' => 'لم يتم العثور على الطلبات المحددة'], 404);
        }
    
        // تحديث حالة الطلبات إلى "ملغاة"
        OrderDetail::whereIn('id', $orderIds)->update(['delivery_status' => 'cancelled','refund_request' => 1]);
    ...
    public function refund_request(Request $request)
    {
        ...
        if (!$order) {
            ...
            //     return response()->json(['error' => 'لم يتم العثور على الطلبات المحددة'], 404);
            // }
            foreach (json_decode($request->items, true) as $detail) {
                $orderDetail = OrderDetail::where('id', $detail['id'])->first(); // استخدام `where` عادي
                if ($orderDetail) {
                    $orderIds[] = $orderDetail->id; // تخزين ID في المصفوفة
                    $orderDetail_price = $orderDetail->price * $detail['qty'];
                    $orderDetail->refund_qty=  $detail['qty'];
                    
                    $orderDetail->save();
                    $detail_amount +=$orderDetail_price ;
                }
                $store = Store::find($order->store_id);
                // $store->notify(new NotificationName($order));

            }
        
            // $refund_amount = round($detail_amount, config('round_up_to_digit'));
            $refund = new Refund();
            $refund->order_id = $order->id;
            $refund->user_id = $order->user_id;
            $refund->order_status = "pending";
            // $refund->refund_status = 'approved';
            $refund->refund_status = 'pending';

            $refund->refund_method = $request->refund_method ??    $order -> payment_method;
            $refund->customer_reason = $request->customer_reason;
            $refund->customer_note = $request->customer_note;
            $refund->refund_amount = $detail_amount;
            $refund->image = $image;


            // $order->order_amount = $order->order_amount - $detail_amount;
            // $order->refund_number = 'refund_number_' . $order->id . '_' . time();
    ...
    public function most_tips(Request $request)
    {
        ...
        try{
            ...
            DB::commit();
                ...
                $data = [
                    ...
                    'order_id' => $order->id,
                    'image' => '',
                    'module_id' => $order->module_id??0,
                    'order_type' => $order->order_type??'',
                    'zone_id' => $order->zone_id??0,
                    'type' => 'new_order',
                ];
